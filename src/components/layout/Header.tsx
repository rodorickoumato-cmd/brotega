  ...
  return (
    <>
      ...
      {mobileOpen && (
        ...
        >
          <div
            className="bg-white w-72 h-full flex flex-col animate-slide-in"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Header professionnel */}
            <div className="bg-[#E63946] text-white px-5 py-4 flex justify-between items-center flex-shrink-0">
              {auth.connecte ? (
                <div className="flex items-center gap-3">
                  <span className="w-10 h-10 rounded-full bg-white/20 text-white text-sm font-black flex items-center justify-center">
                    {auth.initiales}
                  </span>
                  <div className="min-w-0">
                    <p className="font-bold text-sm leading-tight">
                      {auth.role === "vendeur" ? "👑 Vendeur"
                       : auth.role === "admin" ? "⚙️ Admin"
                       : auth.role === "livreur" ? "🏍️ Livreur"
                       : "👤 Client"}
                    </p>
                    <p className="text-white/70 text-xs truncate">Menu personnel</p>
                  </div>
                </div>
              ) : (
                <span className="font-black text-lg">📱 Menu</span>
              )}
              <button onClick={() => setMobileOpen(false)} aria-label="Fermer le menu" className="p-1 rounded-lg hover:bg-white/20 transition-colors">
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <nav className="flex flex-col gap-0">
              ...
              {auth.connecte && (
                <div className="mb-1">
                  <p className="text-xs font-black text-gray-400 uppercase tracking-wider px-3 py-2">Mon Compte</p>
                  <div className="space-y-0">
                    {mobileMenuItems
                      .filter((item) => (item as any).section === "compte")
                      .map((item) => (
                        <Link
                          key={item.href}
                          href={item.href}
                          onClick={() => setMobileOpen(false)}
                          className="py-3 px-4 text-sm font-medium text-gray-700 hover:text-[#E63946] hover:bg-gray-50 transition-colors block border-b border-gray-100 last:border-0"
                        >
                          {item.label}
                        </Link>
                      ))}
              ...
              {/* Section Rôles */}
              {auth.connecte && (
                <div className="mb-1 border-t border-gray-200 pt-2">
                  <p className="text-xs font-black text-gray-400 uppercase tracking-wider px-3 py-2">Mes Activités</p>
                  <div className="space-y-0">
                    {mobileMenuItems
                      .filter((item) => (item as any).section === "roles")
                      .map((item) => (
                        <Link
                          key={item.href}
                          href={item.href}
                          onClick={() => setMobileOpen(false)}
                          className="py-3 px-4 text-sm font-bold text-gray-700 hover:text-[#E63946] hover:bg-[#FEF2F2] transition-colors block border-b border-gray-100 last:border-0"
                        >
                          {item.label}
                        </Link>
                      ))}
              ...
              <div className="mt-4 pt-4 border-t-2 border-gray-200">
                <p className="text-xs font-black text-gray-400 uppercase tracking-wider px-3 py-2">🛍️ Parcourir</p>
                <div className="grid grid-cols-3 gap-2 px-2">
                  {categories.map((c) => (
                    <Link
                      ...
                      onClick={() => setMobileOpen(false)}
                      className="flex flex-col items-center gap-1.5 p-2 rounded-xl hover:bg-[#FEF2F2] transition-colors"
                    >
                      <div className="w-12 h-12 rounded-xl overflow-hidden border border-gray-100 hover:border-[#E63946] transition-colors">
            ...
            {/* Footer du menu */}
            <div className="border-t border-gray-200 px-4 py-3 text-center text-xs text-gray-500 flex-shrink-0">
              <p>J'adore la Famille • Marketplace</p>
            </div>
          </div>
        </div>
      )}
