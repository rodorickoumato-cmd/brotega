...
// Routes accessibles uniquement si connecté (n'importe quel rôle)
const ROUTES_AUTH = ["/compte", "/checkout", "/vendor/register"];

// Routes par rôle (nécessitent le rôle exact)
...
export async function proxy(request: NextRequest) {
  ...
  // Vérification du rôle si connecté sur route restreinte
  if (user) {
    const needsRole =
    ...
    // Redirige un vendeur qui accède à /vendor/register vers le dashboard
    ...
    // Redirige utilisateur déjà connecté hors des pages auth
