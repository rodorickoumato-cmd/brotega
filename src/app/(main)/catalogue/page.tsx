...
function CatalogueContent() {
  ...
  const filtered = useMemo(() => {
    ...
    return list;
  }, [selectedCat, sort, priceRange, city]);

  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      <div className="flex items-center gap-2 text-sm text-gray-500 mb-6">
        <a href="/" className="hover:text-[#00A550]">Accueil</a>
        <span>/</span>
          ...
          <div className="bg-white rounded-2xl p-5 space-y-6 sticky top-24 shadow-sm border border-gray-100">
            ...
            <div>
              ...
              >
                ...
                {["Libreville", "Port-Gentil", "Oyem", "Lambaréné", "Franceville"].map((c) => (
        ...
        <div className="flex-1">
          <div className="flex items-center justify-between mb-5 bg-white rounded-2xl px-4 py-3 shadow-sm border border-gray-100">
            <p className="text-sm text-gray-600 font-medium">
              <span className="text-[#00A550] font-bold">{filtered.length}</span> produits trouvés
            </p>
            <select
              value={sort}
          ...
          {filtered.length === 0 ? (
            <div className="bg-white rounded-2xl p-12 text-center">
              ...
              <button onClick={() => { setSelectedCat(""); setPriceRange([0, 500000]); setCity(""); }} className="mt-4 text-[#00A550] font-semibold hover:underline text-sm">
                Effacer les filtres
              </button>
            </div>
          ) : (
