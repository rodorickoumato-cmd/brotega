...
const STATUT_STYLE: Record<string, { label: string; cls: string }> = {
  ...
  payee_escrow:        { label: "Payée",        cls: "bg-blue-100 text-green-700"    },
  confirmee_vendeur:   { label: "Confirmée",    cls: "bg-blue-100 text-green-700"    },
  ...
  return (
    <div className="min-h-screen bg-[#F7F8FA]">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="min-w-0">
            ...
            <span className={`mt-1.5 inline-block text-xs font-bold px-2.5 py-0.5 rounded-full ${
              ...
              : role === "admin" ? "bg-green-500 text-white"
      ...
      <div className="px-4 -mt-4 space-y-4 pb-10">
        ...
        {isAdmin && (
          <Link href="/admin"
            className="flex items-center gap-3 bg-green-50 border-2 border-green-300 rounded-2xl px-4 py-4 active:scale-95 transition-transform">
            <span className="text-2xl">⚙️</span>
            <div className="flex-1 min-w-0">
              <p className="text-sm font-black text-green-700">Administration</p>
              <p className="text-xs text-green-600">Dashboard admin</p>
            </div>
            <span className="text-green-400 text-lg flex-shrink-0 font-black">→</span>
          </Link>
        )}
