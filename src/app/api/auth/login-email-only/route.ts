...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // ✅ CHECK 2: Legacy user - Use Supabase Admin API directly
    try {
      const { data: users } = await (admin.auth.admin as any).listUsers({
        pageSize: 1000,
      });

      const legacyUser = users?.find((u: any) => u.email === validEmail);
      ...
      console.error("[LOGIN-EMAIL-ONLY] Admin listUsers error:", err);
    }
